...
function onMemberChange() {
  ...
  const coRanking = sortRankingWithTies(Object.entries(coCounts).map(([n,c])=>({name:n,count:c})), groups[selectedGroup]||[])
  ...
  stagesSorted.forEach(st => {
    ...
    stageRanking[st] = sortRankingWithTies(Object.entries(counts).map(([n,c])=>({name:n,count:c})), groups[selectedGroup]||[])
      .map(p=>[`${p.rank}位`,p.name,`${p.count}回`]);
  });
  ...
  Object.keys(yearCounts).forEach(y=>{
    ...
    yearRanking[y] = sortRankingWithTies(Object.entries(counts).map(([n,c])=>({name:n,count:c})), groups[selectedGroup]||[])
      .map(p=>[`${p.rank}位`,p.name,`${p.count}回`]);
  });
