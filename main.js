...
function onMemberChange(){
  ...
  function processMemberData(){
    ...
    // 演目別出演回数ランキング
    const stageRanking = Object.entries(stageCountMap).sort((a,b)=>b[1]-a[1]).map(([s,c])=>[truncateStageNameLong(s),`${c}回`]);
    html+=`<h3>演目別出演回数ランキング</h3>${createTableHTML(['演目','回数'],stageRanking,'stage-rank-table',['stage-column-20',''])}`;
